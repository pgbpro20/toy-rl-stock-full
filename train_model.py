...
# Obviously, in case of mini-batch gradient descent
# BATCH_SIZE is the number of samples to process before model is updated
BATCH_SIZE = 32
# BARS_COUNT is the number of bars included in any single observation
BARS_COUNT = 10
# net and tgt_net are sync'd after a certain number of frames
TARGET_NET_SYNC = 1000
# Data for training
DEFAULT_STOCKS = "data/YNDX_160101_161231.csv"
# Data for validation.
# TODO: Does it really make sense to have these two flipped?
DEFAULT_VAL_STOCKS = "data/YNDX_150101_151231.csv"
# Discount factor
GAMMA = 0.99

# This is the maximum number of observations stored in the buffer
REPLAY_SIZE = 100000
# how many observations are taken before the first time the model begins training
REPLAY_INITIAL = 10000

# This is passed in as "steps_count" to the experience buffer,
# But I'm not sure...
REWARD_STEPS = 2

# Classic learning rate as used by NN optimizers, in this case, ADAM
# ADABOUND may be a better choice overall
LEARNING_RATE = 0.0001

# How many states will be evaluated while training the model?
STATES_TO_EVALUATE = 1000
# How many training "steps" (runs through the while loop below, in this case)
# Does it take before adding model metrics to the writer
EVAL_EVERY_STEP = 1000

# These are to gradually reduce epsilon in our epsilon-greedy action selection
EPSILON_START = 1.0
EPSILON_STOP = 0.1
...
# VALIDATE the model after this many steps
VALIDATION_EVERY_STEP = 100000


if __name__ == "__main__":
    # This section just handles command line arguments
    ...
    # handle if cuda is turned on
    device = torch.device("cuda" if args.cuda else "cpu")

    # This section handles loading and saving data
    # TODO: Simplify this code section
    ...
    if args.year is not None or os.path.isfile(args.data):
        if args.year is not None:
            ...
            # Yummy! Hardcoded "instrument" (as in financial instrument) data
            stock_data = {"YNDX": data.load_relative(args.data)}
    # We start to see that the environment being created. Notes on the environment are available
    # in the environ itself.
    ...
    # Simple deep neural net model used to approximate V(s) and Q(s, a)
    net = models.SimpleFFDQN(env.observation_space.shape[0], env.action_space.n).to(device)
    # Just a deep copy of the network
    tgt_net = ptan.agent.TargetNet(net)
    # Initializing the Epsilon Greedy Action Selector
    # We'll change the epsilon value in the main loop
